...
if "!_PYD_FOUND!"=="0" (
    ...
    echo        Both engines need this module.
    echo.
)
set "_PYD2_FOUND=0"
for %%f in ("%SRC_DIR%python\ghidra\sleigh\decompiler_native*.pyd") do set "_PYD2_FOUND=1"
if "!_PYD2_FOUND!"=="0" (
    echo [WARN] decompiler_native.pyd not found. Run cpp\build.bat first to compile it.
    echo        The C++ decompiler window will not work without it.
    echo.
)
...
echo.
echo   Press F6 in IDA to dual decompile (C++ + Python)!
echo ========================================
echo.
